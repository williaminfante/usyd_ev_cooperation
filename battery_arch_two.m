...
for day=1:my.OBSERVATION_YEARS*my.DAYS_IN_A_YEAR
    ...
    for hour=0:(my.HOURS_IN_A_DAY-1)
        ...
        s_mx(mx_no).discount_counter        = batt(cust(v_id).ba).discount_counter;
        s_mx(mx_no).low_int_counter         = batt(cust(v_id).ba).low_int_counter; 
        s_mx(mx_no).high_int_counter        = batt(cust(v_id).ba).high_int_counter;         
